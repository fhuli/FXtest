...
AddEventHandler('es:playerLoaded', function(source, user)
    ...
    --balances[source] = user.getMoney()

    TriggerClientEvent('banking:updateBalance', source, user.getBank())
    --TriggerClientEvent('banking:updateBalance', source, user.getMoney())
end)
  ...
  TriggerEvent('es:getPlayerFromId', source, function(user)
    if notAllowedToDeposit[source] == nil then
      ...
      if(string.len(rounded) >= 9) then
        ...
        TriggerClientEvent('es_freeroam:notify', source, "~r~You cannot deposit recently stolen money, please wait 5 minutes.")
    end
  end)
end)
