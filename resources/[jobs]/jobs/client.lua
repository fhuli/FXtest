local jobmenu = {
  opened = false,
  title = "Employment Center", -- ENGLISH : "Employment Center"
  currentmenu = "main",
  lastmenu = nil,
  currentpos = nil,
  selectedbutton = 0,
  ...
  menu = {
    ...
    font = 0,
    ["main"] = {
      title = "Choose your job", -- ENGLISH : Choise your job
      name = "main",
      buttons = {
        {title = "Minor", name = "Minor", id = 4}, -- ENGLISH : Minor
        {title = "On the dole", name = "On the dole", id = 1},
        {title = "Taxi Driver", name = "Taxi Driver", id = 5},
...
function ShowJobBlips(bool)
  if bool and #jobmenu_blips == 0 then
    for station,pos in pairs(jobmenu_locations) do
      ...
      AddTextComponentString('Employment Center')
    ...
    Citizen.CreateThread(function()
      while #jobmenu_blips > 0 do
        ...
        for i,b in ipairs(jobmenu_blips) do
            ...
            DisplayHelpText("Enter the ~g~ENTER~s~ Gaff")
            currentlocation = b
            inrange = true
          end
        end
...
RegisterNetEvent('jobssystem:promoteUser')
AddEventHandler('jobssystem:promoteUser', function(promote)
  SendNUIMessage({
    updateJob = true,
    job = nameJob
  })
