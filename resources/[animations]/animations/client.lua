		...
		if DoesEntityExist( ped ) and not IsEntityDead( ped ) and not IsPedInAnyVehicle(PlayerPedId(), true) and CheckSkin(ped) then
			...
			if not IsPauseMenuActive() then
				...
				if IsDisabledControlJustReleased( 0, 246 ) then -- Y
					...
					if IsDisabledControlJustPressed( 0, 246 ) and CheckSkin(ped) then-- Y
 ...
 Citizen.CreateThread(function()
	while true do
		...
		if DoesEntityExist( ped ) and not IsEntityDead( ped ) and not IsPedInAnyVehicle(PlayerPedId(), true) then
			...
			if CheckWeapon(ped) then
				if holstered then
					TaskPlayAnim(ped, "combat@reaction_aim@pistol", "-0", 10.0, 3.0, -1, 50, 10, 0, 0, 0 )
					Citizen.Wait(600)
					ClearPedTasks(ped)
					holstered = false
				end
			elseif not CheckWeapon(ped) then
				if not holstered then
					TaskPlayAnim(ped, "reaction@intimidation@1h", "outro", 10.0, 2.0, -1, 48, 10, 0, 0, 0 )
					Citizen.Wait(700)
					ClearPedTasks(ped)
					holstered = true
				end
