...
Citizen.CreateThread(function()
	while true do
		...
		for _,theWeapon in ipairs(BlacklistedWeapons) do
			Wait(1)
			if HasPedGotWeapon(PlayerPedId(),GetHashKey(theWeapon),false) == 1 and not IsPlayerAceAllowed (source,"anticheese.bypass") then
...
Citizen.CreateThread(function()
	while true do
		...
		if playerPed and not IsPlayerAceAllowed (source,"anticheese.bypass") then
